...
CREATE PROCEDURE [dbo].[usp_DT_LocalPromote_ProcessV3]
AS
BEGIN
	SET NOCOUNT ON;
	...
	[PHOTO_URL] [varchar](4000) NULL,
	[VIDEO_URL] [varchar](4000) NULL,
	[ATTRIBUTION_URL] [varchar](1000) NULL,
	[MENU_URL] [varchar](1000) NULL,
	[PRODUCTS_URL] [varchar](1000) NULL,
	[EVENTS_URL] [varchar](1000) NULL,
	[BIOS_URL] [varchar](1000) NULL,
	[RESERVATION_URL] [varchar](8000) NULL,
	[ORDER_URL] [varchar](1000) NULL,
	[SPECIAL_OFFER_URL] [varchar](1000) NULL,
	[FACEBOOK_URL] [varchar](1000) NULL,
	[AUDIO_URL] [varchar](1000) NULL,
	[COUPON_URL] [varchar](1000) NULL,
	[REVIEW_URL] [varchar](1000) NULL,
	[MAP_URL] [varchar](1000) NULL,
	[PHONE_TRACKING_URL] [varchar](1000) NULL,
	[RATING_IMAGE_URL] [varchar](1000) NULL,
	...
	[WebContent_URL] [varchar](1000) NULL,
	[AdultFlag] [bit] NULL,
	...
	select	
		...
		,ag.Logo_URL
		,ah.Photo_URL
		,REPLACE(am.Video_URL,'&amp;','&') AS Video_URL
		,al.SpecialOffer_URL
		,a.szEmail as Email
		,case when ha.[24hrs]=1 then 'Open 24/7' else null end as Hours
		,coalesce(ha.szComment,hb.szComment) as HoursAdditional
		,case when ISDATE(hb.szOpenHour)=1 THEN '<dayHours day="MON"><interval start="'+ cast(cast(hb.szOpenHour as time(0)) as varchar(5)) +'" end="'+case when ISDATE(hb.szCloseHour)=1 THEN cast(cast(hb.szCloseHour as time(0)) as varchar(5)) ELSE '' end+'" /></dayHours>' ELSE NULL END as [HoursMONStartEnd]
		,case when ISDATE(hc.szOpenHour)=1 THEN '<dayHours day="TUE"><interval start="'+ cast(cast(hc.szOpenHour as time(0)) as varchar(5)) +'" end="'+case when ISDATE(hc.szCloseHour)=1 THEN cast(cast(hc.szCloseHour as time(0)) as varchar(5)) ELSE '' end+'" /></dayHours>' ELSE NULL END as [HoursTUEStartEnd]
		,case when ISDATE(hd.szOpenHour)=1 THEN '<dayHours day="WED"><interval start="'+ cast(cast(hd.szOpenHour as time(0)) as varchar(5)) +'" end="'+case when ISDATE(hd.szCloseHour)=1 THEN cast(cast(hd.szCloseHour as time(0)) as varchar(5)) ELSE '' end+'" /></dayHours>' ELSE NULL END as [HoursWEDStartEnd]
		,case when ISDATE(he.szOpenHour)=1 THEN '<dayHours day="THU"><interval start="'+ cast(cast(he.szOpenHour as time(0)) as varchar(5)) +'" end="'+case when ISDATE(he.szCloseHour)=1 THEN cast(cast(he.szCloseHour as time(0)) as varchar(5)) ELSE '' end+'" /></dayHours>' ELSE NULL END as [HoursTHRStartEnd]
		,case when ISDATE(hf.szOpenHour)=1 THEN '<dayHours day="FRI"><interval start="'+ cast(cast(hf.szOpenHour as time(0)) as varchar(5)) +'" end="'+case when ISDATE(hf.szCloseHour)=1 THEN cast(cast(hf.szCloseHour as time(0)) as varchar(5)) ELSE '' end+'" /></dayHours>' ELSE NULL END as [HoursFRIStartEnd]
		,case when ISDATE(hg.szOpenHour)=1 THEN '<dayHours day="SAT"><interval start="'+ cast(cast(hg.szOpenHour as time(0)) as varchar(5)) +'" end="'+case when ISDATE(hg.szCloseHour)=1 THEN cast(cast(hg.szCloseHour as time(0)) as varchar(5)) ELSE '' end+'" /></dayHours>' ELSE NULL END as [HoursSATStartEnd]
		,case when ISDATE(hh.szOpenHour)=1 THEN '<dayHours day="SUN"><interval start="'+ cast(cast(hh.szOpenHour as time(0)) as varchar(5)) +'" end="'+case when ISDATE(hh.szCloseHour)=1 THEN cast(cast(hh.szCloseHour as time(0)) as varchar(5)) ELSE '' end+'" /></dayHours>' ELSE NULL END as [HoursSUNStartEnd]
		,g1.BDC as BDC1
	...
	outer apply	( SELECT '['+x.szFullLocation+ISNULL(' '+x.szPhotoDesc,'')+']' FROM tblLP_BusPhotoFile x (nolock)
					where x.lAdvertiserID=a.lAdvertiserID and x.sActive='Y' and x.sPhotoType='LGO'
					for xml path('')) as ag(Logo_URL)
	outer apply	( SELECT '['+x.szFullLocation+ISNULL(' '+x.szPhotoDesc,'')+']' FROM tblLP_BusPhotoFile x (nolock)
					where x.lAdvertiserID=a.lAdvertiserID and x.sActive='Y' and x.sPhotoType='PTO'
					for xml path('')) as ah(Photo_URL)
	--outer apply	(select stuff(( SELECT '['+x.szFullLocation+ISNULL(' '+x.szPhotoDesc,'')+']' FROM tblLP_BusPhotoFile x (nolock)
	--				where x.lAdvertiserID=a.lAdvertiserID and x.sActive='Y' and x.sPhotoType='LGO'
	...
	outer apply	( SELECT '['+x.szURL+ISNULL(' '+x.szDisplayURL,'')+']' FROM tblLP_BusSpecialOfferURL x (nolock)
					where x.lAdvertiserID=a.lAdvertiserID and x.sActive='Y' and ISNULL(x.szURL,'')<>''
					for xml path('')) as al(SpecialOffer_URL)
	outer apply	( SELECT '['+REPLACE(v.szBusVideoAddress,' ','&nbsp;')+ISNULL(' '+ISNULL(x.szVideoDesc,x.szVideoTitle),'')+']' FROM tblLP_BusVideoInfo x (nolock)
					JOIN tblLP_BusVideoAddress v (nolock) ON (x.lBusVideoInfoId=v.lBusVideoInfoId)
					where x.lAdvertiserID=a.lAdvertiserID and v.dtActiveEnd is null and ISNULL(v.szBusVideoAddress,'')<>''
					for xml path('')) as am(Video_URL)
	...
	set		a.agencyid = 'AF'
	from	tblLP_ListingProcessV3 a
	left join	tblLP_ProductOrder b on (a.advertiserid = b.ladvertiserid and b.sActive='Y')
	where	b.lAdvertiserID IS NULL AND a.AgencyID = 'AP'
	...
	*********************/
	...
	UPDATE	a
	SET		a.WebSite_URL= CASE WHEN CHARINDEX('://',WebSite_URL) = 0 AND LEN(WebSite_URL)>3 THEN 'http://'+a.WebSite_URL ELSE a.WebSite_URL END
			--,a.Photo_URL=CASE WHEN CHARINDEX('://',Photo_URL) = 0 and len(Photo_URL)>3 THEN 'http://'+a.Photo_URL ELSE a.Photo_URL END
			--,a.video_URL=CASE WHEN CHARINDEX('://',video_URL) = 0 and len(video_URL)>3 THEN 'http://'+a.video_URL ELSE a.video_URL END
			,a.ATTRIBUTION_URL=CASE WHEN CHARINDEX('://',ATTRIBUTION_URL) = 0 and len(ATTRIBUTION_URL)>3  THEN 'http://'+a.ATTRIBUTION_URL ELSE a.ATTRIBUTION_URL END
			,a.MENU_URL=CASE WHEN CHARINDEX('://',MENU_URL) = 0 and len(MENU_URL)>3  THEN 'http://'+a.MENU_URL ELSE a.MENU_URL END
			,a.PRODUCTS_URL=CASE WHEN CHARINDEX('://',PRODUCTS_URL) = 0 and len(PRODUCTS_URL)>3  THEN 'http://'+a.PRODUCTS_URL ELSE a.PRODUCTS_URL END
			,a.EVENTS_URL=CASE WHEN CHARINDEX('://',EVENTS_URL) = 0 and len(EVENTS_URL)>3  THEN 'http://'+a.EVENTS_URL ELSE a.EVENTS_URL END
			,a.BIOS_URL=CASE WHEN CHARINDEX('://',BIOS_URL) = 0 and len(BIOS_URL)>3  THEN 'http://'+a.BIOS_URL ELSE a.BIOS_URL END
			,a.RESERVATION_URL=CASE WHEN CHARINDEX('://',RESERVATION_URL) = 0 and len(RESERVATION_URL)>3  THEN 'http://'+a.RESERVATION_URL ELSE a.RESERVATION_URL END
			,a.ORDER_URL=CASE WHEN CHARINDEX('://',ORDER_URL) = 0 and len(ORDER_URL)>3  THEN 'http://'+a.ORDER_URL ELSE a.ORDER_URL END
			--,a.SPECIAL_OFFER_URL=CASE WHEN CHARINDEX('://',SPECIAL_OFFER_URL) = 0 and len(SPECIAL_OFFER_URL)>3  THEN 'http://'+a.SPECIAL_OFFER_URL ELSE a.SPECIAL_OFFER_URL END
			,a.FACEBOOK_URL=CASE WHEN CHARINDEX('://',FACEBOOK_URL) = 0 and len(FACEBOOK_URL)>3  THEN 'http://'+a.FACEBOOK_URL ELSE a.FACEBOOK_URL END
			,a.AUDIO_URL=CASE WHEN CHARINDEX('://',AUDIO_URL) = 0 and len(AUDIO_URL)>3  THEN 'http://'+a.AUDIO_URL ELSE a.AUDIO_URL END
			,a.COUPON_URL=CASE WHEN CHARINDEX('://',COUPON_URL) = 0 and len(COUPON_URL)>3  THEN 'http://'+a.COUPON_URL ELSE a.COUPON_URL END
			,a.REVIEW_URL=CASE WHEN CHARINDEX('://',REVIEW_URL) = 0 and len(REVIEW_URL)>3  THEN 'http://'+a.REVIEW_URL ELSE a.REVIEW_URL END
			,a.MAP_URL=CASE WHEN CHARINDEX('://',MAP_URL) = 0 and len(MAP_URL)>3  THEN 'http://'+a.MAP_URL ELSE a.MAP_URL END
			,a.PHONE_TRACKING_URL=CASE WHEN CHARINDEX('://',PHONE_TRACKING_URL) = 0 and len(PHONE_TRACKING_URL)>3  THEN 'http://'+a.PHONE_TRACKING_URL ELSE a.PHONE_TRACKING_URL END
			,a.RATING_IMAGE_URL=CASE WHEN CHARINDEX('://',RATING_IMAGE_URL) = 0 and len(RATING_IMAGE_URL)>3  THEN 'http://'+a.RATING_IMAGE_URL ELSE a.RATING_IMAGE_URL END
			,a.TRACKING_URL=CASE WHEN CHARINDEX('://',TRACKING_URL) = 0 and len(TRACKING_URL)>3  THEN 'http://'+a.TRACKING_URL ELSE a.TRACKING_URL END
			,a.PROFILE_URL=CASE WHEN CHARINDEX('://',PROFILE_URL) = 0 and len(PROFILE_URL)>3  THEN 'http://'+a.PROFILE_URL ELSE a.PROFILE_URL END
			,a.WebContent_URL=CASE WHEN CHARINDEX('://',WebContent_URL) = 0 and len(WebContent_URL)>3  THEN 'http://'+a.WebContent_URL ELSE a.WebContent_URL END
	FROM	tblLP_ListingProcessV3 a
	where	a.TermCode is NULL

	UPDATE	a
	SET		a.WebSite_URL=replace(REPLACE('['+a.WebSite_URL+']','|',']['),'[]','')
			--,a.Photo_URL=replace(REPLACE('['+a.Photo_URL+']','|',']['),'[]','')
			--,a.video_URL=replace(REPLACE('['+a.video_URL+']','|',']['),'[]','')
			,a.ATTRIBUTION_URL=replace(REPLACE('['+a.ATTRIBUTION_URL+']','|',']['),'[]','')
			,a.MENU_URL=replace(REPLACE('['+a.MENU_URL+']','|',']['),'[]','')
			,a.PRODUCTS_URL=replace(REPLACE('['+a.PRODUCTS_URL+']','|',']['),'[]','')
			,a.EVENTS_URL=replace(REPLACE('['+a.EVENTS_URL+']','|',']['),'[]','')
			,a.BIOS_URL=replace(REPLACE('['+a.BIOS_URL+']','|',']['),'[]','')
			,a.RESERVATION_URL=replace(REPLACE('['+a.RESERVATION_URL+']','|',']['),'[]','')
			,a.ORDER_URL=replace(REPLACE('['+a.ORDER_URL+']','|',']['),'[]','')
			--,a.SPECIAL_OFFER_URL=replace(REPLACE('['+a.SPECIAL_OFFER_URL+']','|',']['),'[]','')
			,a.FACEBOOK_URL=replace(REPLACE('['+a.FACEBOOK_URL+']','|',']['),'[]','')
			,a.AUDIO_URL=replace(REPLACE('['+a.AUDIO_URL+']','|',']['),'[]','')
			,a.COUPON_URL=replace(REPLACE('['+a.COUPON_URL+']','|',']['),'[]','')
			,a.REVIEW_URL=replace(REPLACE('['+a.REVIEW_URL+']','|',']['),'[]','')
			,a.MAP_URL=replace(REPLACE('['+a.MAP_URL+']','|',']['),'[]','')
			,a.PHONE_TRACKING_URL=replace(REPLACE('['+a.PHONE_TRACKING_URL+']','|',']['),'[]','')
			,a.RATING_IMAGE_URL=replace(REPLACE('['+a.RATING_IMAGE_URL+']','|',']['),'[]','')
			,a.TRACKING_URL=replace(REPLACE('['+a.TRACKING_URL+']','|',']['),'[]','')
			,a.PROFILE_URL=replace(REPLACE('['+a.PROFILE_URL+']','|',']['),'[]','')
			,a.WebContent_URL=replace(REPLACE('['+a.WebContent_URL+']','|',']['),'[]','')
	FROM	tblLP_ListingProcessV3 a
	WHERE	a.TermCode is NULL
	...
	***************************/
	...
	set		a.sAgencyID = b.AgencyId,
			...
			a.saction = case when Termcode is null then 'TRN'
						...
						END,
			a.sSource = CASE WHEN a.sSource='SSF' and b.AgencyID='AP' THEN 'SSP' 
							 WHEN a.sSource='WEB' and b.AgencyID='AP' THEN 'WEP'
							 WHEN a.sSource='SSP' and b.AgencyID='AF' THEN 'SSF'
							 WHEN a.sSource='WEP' and b.AgencyID='AF' THEN 'WEB'
							 ELSE a.sSource
						END	 
	...
	***************************/
	...
	-- ListingStore
		...
		set	
			...
			a.[RESERVATION_URL]= case when b.[AgencyID]='AF' then a.[RESERVATION_URL] else b.[RESERVATION_URL] end,
