...
CREATE PROCEDURE [dbo].[usp_LC_GET_Search_Listing_onLC]  
...
@thePartnerID	int,
@theSiteID		int,
@owner_condition varchar(1000)	-- temp str
...
CREATE TABLE #Results   (
	...
	[listingDeletionRequested] [char](1)	NULL
) 
...
SELECT @owner_condition ='case when  exists (select o.sActive from tblLP_ProductOrder o   where o.lAdvertiserID = pl.lAdvertiserID and o.sActive = ''Y'')
			       ...
			       when exists (select o.sActive from tblLP_ProductOrder o   where o.lAdvertiserID = pl.lAdvertiserID and o.sActive = ''N'')
						 and ps.lSiteID <> '+CONVERT(varchar(25),@logged_in_user_lSiteID)+' and pl.sListingType = ''PAY'' 
				   then ''N''
				   when (ps.lSiteID <> '+CONVERT(varchar(25),@logged_in_user_lSiteID)+' and pl.sListingType in (''LED'', ''FRE'')) then ''N''
				   else ''N''
			   end'
IF @security = 3 
BEGIN
	...
	WHERE lMemberID = @lMemberID AND pt.sActive = 'Y'
	-- end of Find partner
	SELECT @owner_condition ='case when  exists (select o.sActive from tblLP_ProductOrder o   where o.lAdvertiserID = pl.lAdvertiserID and o.sActive = ''Y'')
					and pl.lSiteID IN (select lSiteID From tblLP_PartnerSite Where lPartnerID = ' + CONVERT(varchar(25),@thePartnerID) + ') 	 
					and ps.lSiteID IN (select lSiteID From tblLP_PartnerSite Where lPartnerID = ' + CONVERT(varchar(25),@thePartnerID) + ') 	 and pl.sListingType = ''PAY'' 
				   then ''Y''
			       when exists (select o.sActive from tblLP_ProductOrder o   where o.lAdvertiserID = pl.lAdvertiserID and o.sActive = ''N'')
						 and ps.lSiteID NOT IN (select lSiteID From tblLP_PartnerSite Where lPartnerID = ' + CONVERT(varchar(25),@thePartnerID) + ') and pl.sListingType = ''PAY'' 
				   then ''N''
				   when (ps.lSiteID NOT IN (select lSiteID From tblLP_PartnerSite Where lPartnerID = ' + CONVERT(varchar(25),@thePartnerID) + ') and pl.sListingType in (''LED'', ''FRE'')) then ''N''
				   else ''N''
			   end'
END
...
SELECT DISTINCT b.lMemberID,  
  ...
  pl.sListingType,
	...
	listingDeletionRequested = case when exists (select 1 from tblCC_acxListing acx 
												join tblCC_acxListingNote acxn 
												on acx.lCCListingID = acxn.lCCListingID 
												join tblLP_PartnerSiteMember psm 
												on acxn.szUpdatedby = CONVERT(varchar(25),psm.lMemberID) 
												where psm.lSiteID = '+CONVERT(varchar(25),@logged_in_user_lSiteID)+ ' 
												and acx.sConfirm = ''N'' 
												and acx.lAdvertiserID = pl.lAdvertiserID) 
								then ''Y''
								else ''N'' end
...
left join dbo.tblLP_PartnerSite   ps on (ps.lSiteID = pl.lSiteID AND pl.sListingType in (''LED'', ''FRE''))  or (ps.lSiteID = pc.lSiteID  AND pl.sListingType = ''PAY'')
left join dbo.tblLP_Partner    p  on p.lPartnerID = ps.lPartnerID 
left join dbo.tblLP_PartnerSiteMember pm on  pm.lSiteID = ps.lSiteID
...
--added 1/14/11 by SW for LCAdmin API
IF @sListingStatus is not null  
BEGIN  
SELECT @sql = @sql + ' and bs.sStatus = ('''+@sListingStatus+''') '    
END   
...
IF  (@Sort = 'lAdvertiserID') OR  (@Sort = 'lMemberID' )
SELECT @orderby = '  lAdvertiserID ' + @direction + ', lMemberID ' + @direction + ' '   
...
select @sql ='
	...
	, [sStatus], [sActive], [isGloballyBasic], [listingDeletionRequested]	'
...
GO
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC] TO [iisLpAdminFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC] TO [iisLpAdminSsUiFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC] TO [iisReviewEngine]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC] TO [iisReviewEngineFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC] TO [lcssui]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC] TO [LocalConnect]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC] TO [web_user]
GO
