...
CREATE PROCEDURE [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP]  
...
BEGIN
	...
	SELECT @owner_condition ='case when  exists (select o.sActive from tblLP_ProductOrder o   where o.lAdvertiserID = pl.lAdvertiserID and o.sActive = ''Y'')
				   ...
				   then ''N''
				   when (ps.lSiteID NOT IN (select lSiteID From tblLP_PartnerSite Where lPartnerID = ' + CONVERT(varchar(25),@thePartnerID) + ') and pl.sListingType = ''FRE'') then ''N''
				   else ''N''
			   end'
END
...
join dbo.tblLP_PartnerSite   ps on (ps.lSiteID = pl.lSiteID)  or (ps.lSiteID = pc.lSiteID  AND pl.sListingType = ''PAY'')
...
GO
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP] TO [iisLpAdminFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP] TO [iisLpAdminSsUiFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP] TO [iisReviewEngine]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP] TO [iisReviewEngineFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP] TO [lcssui]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP] TO [LocalConnect]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Search_Listing_onLC_DGLP] TO [web_user]
GO
