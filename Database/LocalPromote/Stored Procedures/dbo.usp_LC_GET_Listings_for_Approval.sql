...
CREATE PROCEDURE [dbo].[usp_LC_GET_Listings_for_Approval]  
 
@logged_in_lMemberID int  , 
@listingType         varchar(10)  = NULL,
@lAdvertiserID int  = NULL, -- mod by Linh, used in authorization component to test if a listing identified by lAdvertiserID needs to be approved.
@lSiteID		int = NULL,
@lPartnerID		int = NULL,
@Sort			varchar(100) = 'szTitle',
...
AND  sActive = 'Y' --Added on 01/04/2010 since there can be multiple records in this table.
...
SELECT DISTINCT  b.lMemberID,  
  ...
  --pl.sListingType,
	ListingType = CASE	WHEN ( po.lAdvertiserID = b.lAdvertiserID 
									AND po.sActive = ''Y'') 
									THEN ''Premium''
						WHEN  (sListingType = ''FRE'' AND pl.dtActiveN IS NULL)
									THEN ''Basic''
				  END,
...
BEGIN
	...
	--Added on 01/04/2010 for filtering on @lSiteID and @lPartnerID for security level 5
	IF @lPartnerID is not NULL and @lSiteID is not NULL
	BEGIN
		SELECT @sql = @sql + ' AND p.lPartnerID = ' + CONVERT(varchar(25), @lPartnerID) + '
							   AND ps.lSiteID = ' + CONVERT(varchar(25), @lSiteID) + ''
	END
	ELSE 
	IF @lPartnerID is not NULL and @lSiteID is NULL
	BEGIN
		SELECT @sql = @sql + ' AND p.lPartnerID = ' + CONVERT(varchar(25), @lPartnerID) + ''
	END
END  
...
IF @lAdvertiserID IS NOT NULL -- -- mod by Linh
	BEGIN
	SELECT @sql = @sql + ' AND b.lAdvertiserID = ' + CONVERT(varchar(25), @lAdvertiserID)+  ''  
	END


--SELECT @sql = @sql + ' ORDER BY  szTitle  asc'  

--Added on 01/04/2010 for ordering 
SELECT @sql = @sql + ' ORDER BY ' + @Sort + ' ' + @Direction + ''
 
--PRINT @sql   
EXEC (@sql)
GO
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Listings_for_Approval] TO [iisLpAdminFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Listings_for_Approval] TO [iisLpAdminSsUiFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Listings_for_Approval] TO [iisReviewEngine]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Listings_for_Approval] TO [iisReviewEngineFull]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Listings_for_Approval] TO [lcssui]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Listings_for_Approval] TO [LocalConnect]
GRANT EXECUTE ON  [dbo].[usp_LC_GET_Listings_for_Approval] TO [web_user]
GO
